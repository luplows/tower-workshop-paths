...
You do not merge your own PR, and neither does the reviewer. [`.github/workflows/land-approved.yml`](.github/workflows/land-approved.yml) lands the oldest open PR that is not a draft, carries no `review-blocked` label, reads `clean` (no conflict, every required check green), and has the `review/agent` commit status reading `success` on its current head SHA. **It runs only when triggered** — `gh workflow run land-approved.yml` — and **triggering it is the owner's to do, not an agent session's**: a coder that dispatches the sweep while its own PR is the oldest eligible one has merged its own PR through the sentence that told it not to. It used to carry a `*/15` schedule, which GitHub delivered at roughly one run every five hours; that trigger is gone until a reliable one is chosen (`stories/OQ-50-land-without-scheduler.md`). So a merge-ready PR waits until somebody lands it, which is expected rather than a fault. One PR per run, then it stops — GitHub recomputes mergeability asynchronously, so a second merge in the same run would be decided on state read before the first one landed. The branch is deleted automatically, so you do not need to — and in a spawned container you could not, as the git proxy refuses ref deletion outright (HTTP 403).
