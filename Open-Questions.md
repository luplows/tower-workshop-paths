...
`.github/workflows/` holds only `ci.yml` and `deploy-pages.yml` — there is no review automation. Since the visual-inspection gate was removed, CI is the sole quality signal on every PR, and CI only knows what the tests already assert; it cannot flag a plausible-looking bug that no test covers. Options: a code-review GitHub Action running on PRs, or a review step in the pre-push routine rather than a CI job. Open questions: which fits better given PRs here are often authored with Claude's help already (a reviewer sharing the author's blind spots is worth less than an independent one); and whether findings should block the merge or only comment.
