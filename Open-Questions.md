...
**OQ-6. Implement Enhancement tree gating**
...
**The Lab half is done (OQ-31):** the one-time "Workshop Enhancements" Lab (5B coins, confirmed) that gates the whole Enhancement system is modeled in the Path list — see `Completed-Questions.md`. **Still open:** the per-tree gate. Unlike Workshop's one-time unlock purchases, an Enhancement *category* unlocks once *cumulative coins spent on that tree's other enhancements* crosses a threshold (50B → 500B → 5T → 50T → 500T — see `Project-Outline.md`), even after the Lab itself is bought. Needs the algorithm to track running per-tree Enhancement spend, not just current levels, and to exclude a category from the Path list until its tree has crossed the relevant threshold — the same "explicitly out of scope for now" gap OQ-29 left for this part specifically.
