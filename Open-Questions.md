...
**What to actually look at, beyond the raw ratio.** Reviewers have repeatedly found real problems and classified them as *observations* rather than failures: a tool named as the primary method that did not exist in the container, an undescribed rider in a diff. Those were right to surface and arguably right not to block, but `pass`/`fail` has no way to say so, which means the verdict record understates what review catches. Worth deciding then whether the marker needs a third state, or whether prose findings on a passing review are good enough.

**OQ-48. Nothing automatically applies `review-blocked` or enforces the 2-round bound**
As someone relying on the circuit breaker, I want the bound that feeds it to be enforced by something other than an agent remembering, so that the breaker is counting a real signal rather than one that gets applied when somebody happens to follow the rule.
OQ-42 decided that review retries are bounded at two rounds, after which the agent states its position, applies a `review-blocked` label and escalates to a human. The automation for that lived in `.github/workflows/review.yml`, gated behind `REVIEW_ENABLED` — which was never turned on, because the review step it gated could not authenticate. So that code never ran, and deleting the file (see `Completed-Questions.md`, OQ-42) removed it.
The bound was honoured exactly once, on [#96](https://github.com/luplows/tower-workshop-paths/pull/96), by an orchestrator following the rule in prose and applying the label by hand. That worked, and it is not a mechanism. Today the `review-blocked` label is only ever applied manually, which means the circuit breaker in `land-approved.yml` and the `SessionStart` hook's banner are both counting a signal that nothing reliably produces.
**Open questions:** where the round count should live, given that a reviewer session is fresh each time and cannot know how many rounds preceded it (the PR's own comment history is the only durable record); whether the labelling belongs in `review-gate.yml`, which already parses verdicts and could count `fail` markers per head SHA; and whether a bound on *rounds* is even the right shape, versus a bound on total review spend per PR.
