...
The game lets you buy multiple levels of a Workshop upgrade in one purchase — it doesn't *require* buying in batches, single-level purchases remain available too — but for this tool's purposes, the buy order is always expressed in fixed batches rather than single levels, for practicality: **100 levels at a time** for a Workshop upgrade with more than 1000 max levels, **10 at a time** for one with at least 10, or **1 at a time** for one with fewer than 10 max levels in the first place (Multishot Targets, Bounce Shot Targets, Orbs). This applies to Workshop upgrades only — Enhancements are always bought **1 level at a time** for the purposes of the buy algorithm. Implemented for the cost-only Path list (`getCheapestNextUpgrades`, OQ-7): a batch's cost is the sum of its consecutive per-level costs, and a batch shrinks to whatever's left when fewer than a full batch remain before max level. If any level within a (max-trimmed) batch has no valid cost data, the whole batch is refused rather than pricing a partial one — see OQ-1. Not yet applied to the priority-weighted algorithm (OQ-2), which doesn't exist yet.
