...
- A phone-mimicking React UI with two levels of navigation:
  ...
  - Within "Input", an "Upgrade" / "Enhance" mode tab bar matching the game's own buttons, switching between:
    - **Upgrade**: enter your current level in every Workshop upgrade, backed by per-level cost data from the [`tower-idle-toolkit`](https://www.npmjs.com/package/tower-idle-toolkit) npm package. Some upgrades are grouped behind a one-time coin cost to unlock, matching the game (e.g. Attack's "Multishot Upgrades" group, 400 coins, unlocks "Multishot Chance" and "Multishot Targets") — a tree's groups unlock in order, so only the next group in line shows an inline "Unlock "{group}" ({cost})" button; a later group's upgrades show a "Locked until "{that group}" is unlocked" note instead, even though they gate unrelated upgrades.
