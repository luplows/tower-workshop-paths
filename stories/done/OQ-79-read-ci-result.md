...
- [x] **AC-1** — A new module, `scripts/dispatch/ci.mjs`, has an entry point
...
- [x] **AC-2** — CI means the runs of `.github/workflows/ci.yml` for that head
...
- [x] **AC-3** — For `red`, the result also carries **findings text** a retry
...
- [x] **AC-4** — A function derives the number of **red CI rounds** on a pull
...
- [x] **AC-5** — `ci.mjs` only reads. It has its own allowlist, checked before
